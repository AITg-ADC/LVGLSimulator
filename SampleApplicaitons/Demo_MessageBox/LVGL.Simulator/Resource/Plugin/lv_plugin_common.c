...
LV_PLUG_RET	lv_plugin_msgbox_update_font(lv_obj_t* msgbox, int16_t part)
{
    ...
    return lv_plugin_obj_update_font(msgbox, string_id, part);
}
...
LV_PLUG_RET	lv_plugin_label_update_font(lv_obj_t* label, int16_t part)
{
    ...
    return lv_plugin_obj_update_font(label, string_id, part);
}
...
LV_PLUG_RET	lv_plugin_obj_update_font(lv_obj_t* obj, lv_plugin_res_id string_id, int16_t part)
{
    ...
    lv_plugin_res_id curr_lang_font = res->language_font_map[inter_param->curr_language_id];

    LV_ASSERT_NULL(res)

    if((string_id < res->string_table_size) && (string_id != LV_PLUGIN_RES_ID_NONE)){

        lv_plugin_res_id language_id = res->fixed_language_string_id_table[string_id];

        if(language_id != LV_PLUGIN_RES_ID_NONE)
            font_id = res->language_font_map[language_id];
        else
            font_id = curr_lang_font;
    }
    else{
        return LV_PLUG_SUCCESS;
    }

    LV_LOG_TRACE("update string id(%u) with font id(%u)", id, font_id);
